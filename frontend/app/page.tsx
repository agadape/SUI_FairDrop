...
const ICheck = (p: { className?: string }) => (
  <Icon className={p.className} d="M20 6 9 17l-5-5" />
);
const IExternal = (p: { className?: string }) => (
  <Icon
    className={p.className}
    d="M15 3h6v6M10 14 21 3M21 14v5a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h5"
  />
);
const IChevron = (p: { className?: string }) => (
  <Icon className={p.className} d="M9 6l6 6-6 6" />
);
const IArrowRight = (p: { className?: string }) => (
  <Icon className={p.className} d="M5 12h14M13 6l6 6-6 6" />
);
const IBolt = (p: { className?: string }) => (
  <Icon className={p.className} d="M13 2 3 14h7l-1 8 10-12h-7l1-8z" />
);
...
const PATH_DICE =
  "M4 4h16v16H4zM8.5 8.5h.01M15.5 8.5h.01M12 12h.01M8.5 15.5h.01M15.5 15.5h.01";
...
const FAIRNESS = [
  {
    ...
    visual: (
      <div className="flex items-center gap-2 font-mono text-xs">
        <span className="text-zinc-500 bg-zinc-50 border border-zinc-100 px-2 py-0.5 rounded-lg text-[10px]">
          Wallet addr
        </span>
        <span className="text-zinc-300">→</span>
        <span className="text-blue-500 text-[10px]">sha3_256</span>
        <span className="text-zinc-300">→</span>
        <span className="text-blue-600 text-[10px] bg-blue-50 px-1.5 py-0.5 rounded-lg border border-blue-100">
          0x4f2a…
        </span>
      </div>
    ),
  },
  {
    ...
    visual: (
      <div className="flex items-center gap-2 font-mono text-xs">
        <span className="text-amber-600 bg-amber-50 px-2 py-0.5 rounded-lg border border-amber-100 text-[10px]">
          5.0 SUI
        </span>
        <span className="text-zinc-300">→</span>
  ...
  },
  {
    ...
    visual: (
      <div className="flex items-center gap-2 font-mono text-xs">
        ...
        <span className="text-zinc-300">→</span>
        <span className="text-violet-600 font-bold px-2 py-0.5 rounded-lg bg-violet-50 border border-violet-100">
          0x8
        </span>
        <span className="text-zinc-300">→</span>
    ...
    visual: (
      <div className="font-mono text-[10px] space-y-1">
        <div className="text-emerald-600">
          ✓ winners → allocation + escrow change
        </div>
...
const STEPS = [
  {
    ...
    desc: "OAuth-gated entry, raising Sybil attack costs. A Google login is required to register, and each address registers exactly once: a per-registration nullifier is committed on-chain, and a second registration from the same address or nullifier is rejected. This raises the cost of multi-wallet farming — not proof-of-personhood.",
  },
  {
    num: "02",
    label: "Seal your bid",
    desc: "You choose an amount and submit sha3_256(amount ‖ nonce). The hash commits you to the bid without revealing it. Miners, validators, and every other bidder see only a 32-byte hash until reveal. Front-running requires seeing the bid. You made that impossible.",
  },
  {
    num: "03",
    label: "Reveal when the phase opens",
    desc: "After the commit window closes, you send your actual amount and nonce. The contract recomputes the hash and verifies it matches exactly. Reveal a different amount: rejected. Reveal late: rejected. No flexibility, by design.",
  },
  {
    num: "04",
    label: "Anyone calls resolve",
    desc: "No admin required. Anyone calls resolve and the contract does the work: sort bids, find the clearing price, break ties using sui::random at 0x8. Validator DKG randomness — no single party can predict or bias it. The outcome is on-chain and permanent.",
  },
  {
    num: "05",
    label: "Settle, then claim",
    desc: "resolve runs once: it sorts bids, finds the clearing price, breaks ties with sui::random, and mints a WinnerCertificate to every winner — all in one PTB. Settlement is then pull-based: winners claim, losers reclaim their full escrow, the creator withdraws proceeds. Each is a separate permissionless call.",
  },
];
...
const PRIMITIVES = [
  {
    name: "Seal",
    by: "Kostas Chalkias",
    desc: "Threshold-encrypts your reveal secret. An on-chain Move policy (seal_approve) releases it only to you, only during reveal — no FairDrop server can decrypt it.",
    color: "text-pink-600",
    dot: "bg-pink-400",
  },
  {
    name: "Walrus",
    by: "Mysten Labs",
    desc: "Decentralized blob storage for the encrypted secret. Recoverable from any device — no backend, no team custody.",
    color: "text-cyan-600",
    dot: "bg-cyan-400",
  },
  {
    name: "zkLogin",
    by: "Deepak Maram",
    desc: "Google login instead of a wallet — the same login re-derives the same wallet on any device. Raises Sybil cost; not proof-of-personhood.",
    color: "text-blue-600",
    dot: "bg-blue-400",
  },
  {
    name: "sui::random",
    by: "Andrew Schran",
    desc: "Validator DKG randomness at object 0x8. Unpredictable before the transaction. Unbiasable by any party.",
  ...
  },
  {
    name: "Enoki",
    by: "Mysten Labs",
    desc: "The zkLogin auth flow behind the Google button — onboards users with no seed phrase and no extension. Gas is self-paid on testnet.",
  ...
  },
  {
    name: "PTBs",
    by: "Sui protocol",
    desc: "Programmable Transaction Blocks compose many Move calls into one atomic transaction — resolve mints every WinnerCertificate in a single PTB. Final settlement is pull-based.",
...
const RECEIPTS = [
  {
    ...
    id: PACKAGE_ID,
    hint: "auction + seal_policy Move source · immutable",
  },
  {
    label: "Live auction object",
    id: AUCTION_ID,
    hint: "commitments, reveals & winners tables · live state",
  },
  {
    label: "sui::random (0x8)",
    id: RANDOM_ID,
    hint: "validator DKG · consumed in every resolve tx",
  },
  {
    label: "Clock (0x6)",
    id: "0x6",
    hint: "timestamp source for phase gating",
  },
];
...
const CARD =
  "rounded-[2rem] bg-white border border-black/[0.04] shadow-[0_20px_60px_-15px_rgba(0,0,0,0.06)]";
const CARD_SM =
  "rounded-2xl bg-white border border-black/[0.04] shadow-[0_8px_24px_-8px_rgba(0,0,0,0.05)]";
const BTN_PRIMARY =
  "inline-flex items-center gap-2 rounded-full px-6 py-3 bg-zinc-900 text-white text-sm font-semibold hover:-translate-y-0.5 hover:shadow-lg transition-all duration-300 active:scale-[0.98]";
const BTN_SECONDARY =
  "inline-flex items-center gap-2 rounded-full px-6 py-3 border border-black/[0.1] bg-white text-zinc-700 text-sm font-semibold hover:-translate-y-0.5 hover:shadow-md transition-all duration-300 active:scale-[0.98]";
const EYEBROW =
  "text-xs text-zinc-400 font-medium uppercase tracking-widest mb-3";
    ...
    <nav className="sticky top-0 z-50 border-b border-black/[0.05] bg-white/80 backdrop-blur-xl">
      <div className="max-w-6xl mx-auto px-6 h-14 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <a
            href="#how"
            className="text-sm text-zinc-500 hover:text-zinc-900 transition-colors px-3 py-1.5 rounded-full hover:bg-zinc-50 hidden md:block"
          >
            How it works
          </a>
          <a
          ...
          <a
            href="/umbra"
            className="inline-flex items-center gap-1.5 text-sm font-semibold text-blue-600 border border-blue-200 rounded-full px-3 py-1.5 hover:bg-blue-50 transition-all duration-200"
          >
            <IBolt className="w-3.5 h-3.5" /> Umbra
          </a>
          <ConnectButton />
        </div>
      </div>
    ...
    <section className="relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-4 pb-10 lg:pt-6 lg:pb-14 grid lg:grid-cols-[1.85fr_1fr] gap-4 lg:gap-5 items-stretch">
        ...
        >
          ...
          >
            <span className="text-transparent bg-clip-text bg-gradient-to-b from-zinc-900 to-zinc-700">
              Lose your device.
            </span>{" "}
            <span className="text-transparent bg-clip-text bg-gradient-to-b from-zinc-500 to-zinc-400">
              Keep your bid.
            </span>
          </motion.h1>
          ...
          >
            One Google login, a blind bid only you can open — held on Walrus,
            locked by Seal. No server, no FairDrop team, ever sees it.
          </motion.p>
          ...
          >
            <a
              href="#auction"
              className="inline-flex items-center gap-2 rounded-full bg-zinc-950 px-6 py-3 text-sm font-semibold text-white hover:-translate-y-0.5 hover:shadow-xl transition-all duration-300 active:scale-[0.98]"
            >
              Launch live auction <IArrowRight className="w-4 h-4" />
            </a>
            {AUCTION_ID && (
              <a
                href={objUrl(AUCTION_ID)}
                target="_blank"
          ...
          <motion.div
            aria-hidden
            className="pointer-events-none absolute bottom-2 -right-[6%] z-0 hidden sm:block h-[70%] w-[50%] lg:h-[80%] lg:w-[53%] bg-[url('/illustrations/mobile-encryption.svg')] bg-contain bg-right-bottom bg-no-repeat opacity-90"
        ...
        <div className="flex flex-col gap-4 lg:gap-5">
          ...
          >
            ...
            <div className="relative z-10 mt-2 h-28">
              <span className="absolute right-9 top-1 z-10 rounded-full bg-yellow-300 px-2.5 py-0.5 text-[11px] font-bold text-zinc-900">
                live
              </span>
              <svg
                viewBox="0 0 340 112"
              ...
              {[
                {
                  dot: "bg-zinc-500",
                  label: "Device lost",
                  note: "localStorage gone",
                },
                {
                  dot: "bg-cyan-400",
                  label: "Walrus",
                  note: "encrypted blob fetched",
                },
                { dot: "bg-rose-400", label: "Seal", note: "policy approved" },
                {
                  dot: "bg-emerald-400",
                  label: "Bid restored",
                  note: "same wallet · zkLogin",
                },
              ].map((s, i, arr) => (
                <div key={s.label} className="flex items-center gap-2.5">
                  <span className="relative flex flex-col items-center">
                    ...
                    {i < arr.length - 1 && (
                      <span className="absolute top-1.5 h-3.5 w-px bg-white/10" />
                    )}
                  </span>
            ...
            <div className="flex items-center gap-3">
              <div className="flex -space-x-2">
                {[
                  {
                    label: "W",
                    ring: "bg-cyan-500/20 border-cyan-400/40 text-cyan-300",
                  },
                  {
                    label: "S",
                    ring: "bg-rose-500/20 border-rose-400/40 text-rose-300",
                  },
                  {
                    label: "zk",
                    ring: "bg-blue-500/20 border-blue-400/40 text-blue-300",
                  },
                ].map((c) => (
                  <span
                    key={c.label}
                    className={`flex h-9 w-9 items-center justify-center rounded-full border text-[11px] font-bold font-mono ${c.ring} ring-2 ring-zinc-950`}
                  >
                    {c.label}
                  </span>
                ))}
      ...
      <div className="max-w-5xl mx-auto px-6 flex flex-col items-center text-center gap-10">
        ...
        >
          ...
          <h2 className="text-3xl lg:text-5xl font-bold text-zinc-800 tracking-tight">
            Lose your device.
            <br className="hidden sm:block" /> Keep your bid.
          </h2>
          <p className="max-w-lg text-zinc-500 text-base leading-relaxed">
            Your encrypted nonce lives in Walrus. Your access policy lives
            on-chain. No server holds your keys — not even ours.
          </p>
        </motion.div>
        ...
        >
          <div className="rounded-[2rem] bg-white border border-zinc-100 shadow-[0_32px_80px_-16px_rgba(0,0,0,0.10),0_0_0_1px_rgba(0,0,0,0.03)] overflow-hidden">
            <div className="flex items-center gap-1.5 px-5 pt-4 pb-3 border-b border-zinc-50">
              ...
              <span className="w-2 h-2 rounded-full bg-zinc-100" />
              <span className="w-2 h-2 rounded-full bg-zinc-100" />
              <span className="ml-3 text-[10px] font-mono text-zinc-400 tracking-[0.15em] uppercase">
                Recovery Flow · Live
              </span>
            </div>
            <div className="p-5">
        ...
        >
          Walrus blob_id stored in Commitment on-chain · Seal policy: Entry
          owner only
        </motion.p>
      </div>
    </section>
    ...
    <section className="relative overflow-hidden py-20">
      ...
      <div className="relative max-w-6xl mx-auto px-6">
        ...
        >
          ...
          <motion.p
            variants={fadeUp}
            className="text-zinc-500 mb-12 max-w-xl leading-relaxed"
          >
            It looks fair from the outside; the mechanics tell a different story
            — bots, whales, and insiders have structural advantages before the
            first bid is placed.
          </motion.p>
              ...
              >
                ...
                <div className="mt-4 pt-4 border-t border-black/[0.04]">
                  <p className="text-[11px] text-emerald-600 leading-relaxed">
                    <span className="font-semibold">FairDrop: </span>
                    {p.fix}
                  </p>
                </div>
              </motion.div>
    ...
    <section id="how" className="relative overflow-hidden py-20">
      ...
      <div className="relative max-w-6xl mx-auto px-6">
        ...
        >
          ...
          <motion.div
            variants={stagger}
            className={`${CARD} overflow-hidden divide-y divide-black/[0.04]`}
          >
            {STEPS.map((step, i) => (
              <motion.div
                key={step.num}
              ...
              >
                <span
                  className={`font-mono text-2xl font-bold flex-shrink-0 leading-none w-10 transition-colors duration-200 ${
                    active === i ? "text-blue-500" : "text-zinc-200"
                  }`}
                >
                  {step.num}
                </span>
      ...
      <div className="relative max-w-6xl mx-auto px-6">
        ...
        >
          ...
          <motion.p
            ...
            className="text-zinc-500 mb-12 max-w-xl leading-relaxed"
          >
            No bridges. No oracles from other chains. Every fairness guarantee
            is enforced by Sui&apos;s own infrastructure.
          </motion.p>
            ...
            {PRIMITIVES.map((p) => (
              ...
              >
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className={`text-sm font-bold ${p.color} font-mono`}>
                    {p.name}
                  </span>
    ...
    <section className="relative overflow-hidden py-20">
      <div className="relative max-w-6xl mx-auto px-6">
        ...
        >
          ...
          <motion.p
            variants={fadeUp}
            className="text-zinc-500 mb-8 max-w-xl leading-relaxed"
          >
            Every object, transaction, and state change is on-chain. Open the
            live objects below on SuiScan and trace any auction&apos;s artifacts
            straight from its history.
          </motion.p>
            ...
            {RECEIPTS.filter((r) => r.id).map((r) => (
              ...
              >
                <div className="min-w-0">
                  <p className="text-sm text-zinc-800 font-medium group-hover:text-blue-600 transition-colors">
                    {r.label}
                  </p>
                ...
                <span className="text-zinc-300 group-hover:text-blue-500 transition-colors text-lg flex-shrink-0 ml-4">
                  ↗
                </span>
              </motion.a>
            ))}
          ...
          <motion.div variants={fadeUp} className={`${CARD_SM} p-5`}>
            ...
            <div className="grid sm:grid-cols-2 gap-x-6 gap-y-2 font-mono text-[11px]">
              {[
                {
                  k: "Commitment",
                  v: "hash only — no amount on-chain until reveal",
                  c: "text-amber-500",
                },
                {
                  k: "BidRevealed event",
                  v: "amount appears here, in the reveal tx",
                  c: "text-amber-500",
                },
                {
                  k: "Resolve tx",
                  v: "consumes 0x8 — selects winners via DKG randomness",
                  c: "text-blue-500",
                },
                {
                  k: "WinnerCertificate",
                  v: "minted on-chain to each winner",
                  c: "text-emerald-500",
                },
              ].map((a) => (
      ...
      <div className="max-w-6xl mx-auto px-6 flex flex-col sm:flex-row justify-between gap-8 text-xs text-zinc-500">
        ...
        <div>
          ...
          <div className="flex flex-wrap gap-x-3 gap-y-1 font-mono text-[10px] text-zinc-400">
            {["zkLogin", "sui::random", "Seal", "Walrus", "Enoki", "Pyth"].map(
              (s) => (
                <span key={s}>{s}</span>
              )
            )}
        ...
        <div className="relative max-w-6xl mx-auto px-6">
          ...
          >
            ...
            <motion.p variants={fadeUp} className="text-zinc-500 text-sm">
              All state is on-chain. Every action is verifiable. No server
              involved.
            </motion.p>
          </motion.div>
          <LiveAuction />
      ...
      <section className="relative overflow-hidden py-28">
        ...
        <div className="relative max-w-6xl mx-auto px-6 text-center">
          ...
          >
            <motion.h2
              variants={fadeUp}
              className="text-4xl lg:text-6xl font-bold text-zinc-900 mb-6 tracking-tight leading-tight"
            >
              Lose your device.{" "}
            ...
            <motion.p
              variants={fadeUp}
              className="text-zinc-500 text-lg mb-10 max-w-lg mx-auto leading-relaxed"
            >
              Blind bids, a verifiable draw, atomic settlement — and a sealed
              bid you can recover from any device. No backend, no admin key, no
              trust beyond the Sui protocol itself.
            </motion.p>
            <motion.div
              variants={fadeUp}
